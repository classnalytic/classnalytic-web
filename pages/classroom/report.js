...
const Report = dynamic(import('../../components/pages/classroom/report'), {
  loading: () => <Loader />
})
    ...
    return (
      <Fragment>
        ...
        <Helmet
          ...
          title='Classnalytic | Report'
          meta={[
            { name: 'viewport', content: 'width=device-width, initial-scale=1, maximum-scale=1, user-scalable=0' },
            { property: 'og:title', content: 'Classnalytic' }
          ]}
        />
        <Protected>{found ? <Report classroom={classroom} /> : !loading && <NotFound />}</Protected>
      </Fragment>
    )
  }
}

export default connect(state => state, { getClassroomDetail, checkLogin })(ClassroomPage)
